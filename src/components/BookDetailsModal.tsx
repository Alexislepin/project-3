...
export function BookDetailsModal({ 
    ...
    (async () => {
      try {
        ...
        if (summaryData?.summary) {
          const txt = summaryData.summary.trim();
          const shouldDiscard =
            txt.length < 80 ||
            /en cours de génération/i.test(txt) ||
            /environ\s+\d+\s*pages/i.test(txt) ||
            /Livre de/i.test(txt);

          if (!shouldDiscard) {
            console.debug('[BookDetailsModal] Summary found - lang:', summaryData.lang, 'summary length:', txt.length);
            setSummary(txt);
            setLoadingSummary(false);
            return;
          } else {
            console.debug('[BookDetailsModal] Summary found but discarded (too short or generic)');
          }
        }

        // No summary found in DB -> attempt to generate via Edge Function
        console.debug('[BookDetailsModal] No summary found, invoking edge function book-summary');
        try {
          const { data: genData } = await supabase.functions.invoke('book-summary', {
            body: {
              source,
              source_id: sourceId,
              title: book.title || '',
          ...
          const generated = (genData as any)?.summary;
          if (generated && typeof generated === 'string' && generated.trim().length > 0) {
            console.debug('[BookDetailsModal] Summary generated by edge function');
            setSummary(generated.trim());
            setLoadingSummary(false);
            return;
          }
        } catch (genErr) {
          console.warn('[BookDetailsModal] Edge function book-summary failed or returned no summary', genErr);
        }
  ...
  useEffect(() => {
    ...
    const loadTranslatedDescription = async () => {
      ...
      try {
        ...
        const isBadText = (txt?: string | null) => {
          if (!txt) return true;
          const t = txt.trim();
          if (t.length < 80) return true;
          if (/en cours de génération/i.test(t)) return true;
          if (/environ\s+\d+\s*pages/i.test(t)) return true;
          if (/Livre de/i.test(t)) return true;
          if (/résumé indisponible/i.test(t)) return true;
          return false;
        };

        const buildRichFallback = () => {
          const title = book.title?.trim() || 'Cet ouvrage';
          const author = book.author || (book as any).authors || 'un auteur';
          const pages = book.total_pages || (book as any).pageCount;
          const year =
            (book as any).firstPublishYear ||
            ((book as any).publishedDate ? new Date((book as any).publishedDate).getFullYear() : null);
          const genre = book.genre || (book as any).category;

          const parts: string[] = [];
          parts.push(`${title} est un livre de ${author}`);
          if (year) parts.push(`publié en ${year}`);
          if (pages) parts.push(`${pages} pages`);
          if (genre) parts.push(`qui explore ${genre.toLowerCase()}`);
          parts.push(`un récit développé avec des thèmes forts. Découvrez l'histoire complète en le lisant.`);
          return parts.join(', ');
        };

        // Priority 1: summary from book_summaries (already in correct language from query)
        if (summary && !isBadText(summary)) {
          ...
          setTranslatedDescription(isBadText(translated) ? buildRichFallback() : translated);
        ...
        if (book.description && book.description.trim().length > 0 && !isBadText(book.description)) {
        ...
        const fallback = buildRichFallback();
  ...
  const footer = (showAddButton || userBookId || onOpenRecap) ? (
    <div className="px-6 pt-4 flex gap-3">
      ...
      {onOpenRecap && (
        <button
          onClick={onOpenRecap}
          className={`py-3 px-4 bg-stone-900 text-white rounded-xl font-semibold hover:brightness-95 transition-all flex items-center justify-center gap-2 ${
            userBookId && onEditRequested ? 'flex-1' : 'flex-1'
          }`}
        >
  ...
  return (
    <ModalSheet onClose={onClose} header={header} footer={footer}>
      <div className="px-6 py-6">
          <div className="flex gap-4 mb-6 items-start">
            <div className="w-20 h-28 shrink-0">
              {(() => {
                // Derive stable bookKey for pooled cover lookup
                const stableBookKey =
